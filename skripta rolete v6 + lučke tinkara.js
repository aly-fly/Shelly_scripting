/*
https://shelly-api-docs.shelly.cloud/gen2/Scripts/ShellyScriptLanguageFeatures#shellycall
https://drive.google.com/file/d/1abS77tX4BW41PDd7aoJcRdXAKt4rFxvu/view
https://shelly-api-docs.shelly.cloud/gen1/#shelly-plug-plugs-relay-0      
*/      
function remoteRelayOnOff (IPaddr, newState) {
  if (typeof IPaddr == "string") { // valid string
    let url = "http://" + IPaddr + "/relay/0/?turn=" + newState;
    if (newState === "on") { // turn off after 1500 seconds
      url = url +  "&timer=1500"
      }
    logger(url, "EXEC");      
    Shelly.call(
...
function getCoverStatusAndExecAction(NightLightIP) {
  Shelly.call("Cover.GetStatus", {"id":0}, function(result) {
    ...
    if ((currState === "opening") || (currState === "closing")) {
      ...
      remoteRelayOnOff(NightLightIP, "off");
      return;
      } 
    if (lastDir === "open") {
      logger("Action = Close", "COVER");
      Shelly.call("Cover.Close", { id: 0 });
      remoteRelayOnOff(NightLightIP, "on");
      return;
      } 
    else { // last dir = close
...
//        address: "60:ef:ab:40:50:16", // Marcel
      ...
      action: function (data) {
        ...
        logger("Scene 1", "EXEC");
        // trigger an action
        getCoverStatusAndExecAction("10.38.22.101"); // Get status, make an action as a callback + control IP of the night light
      },
    },
    /** SCENE END 1 **/
  ],
