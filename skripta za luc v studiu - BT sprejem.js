...
let CONFIG = {
  ...
  debug: true,
  // BT motion sensor address
  expectedDeviceAddress : "b0:c7:de:11:5b:f8",
  // name of the event that is emitted when address matches
  eventName: "BLU-DATA",
  }
/******************* STOP CHANGE HERE *******************/
...
//    logger("Missing service_data member", "Error");
  ...
  logger(["Shelly BLU device seen", JSON.stringify(unpackedData)], "Info");
  
  if (unpackedData.address === CONFIG.expectedDeviceAddress) {
    logger("BT Device address matches.", "Info");

  emitData(unpackedData);
  }
}

// Emitting the decoded BLE data to a specified event. It allows other scripts to receive and process the emitted data
